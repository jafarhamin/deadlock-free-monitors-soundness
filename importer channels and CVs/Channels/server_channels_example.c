...
predicate_ctor Ms(channel s)(pair<int, channel> message) = 
  [_]channel(snd(message),Mch'(s),(M'ch')(s),U) &*&
  Mr(snd(message)) == nil &*& 
...
void server(struct channel *s)
  //@ requires obs(nil, cons(s,nil)) &*& [_]channel(s,Ms(s),(M's)(s),U) &*& Mr(s) == cons(1r,nil) &*& Ser(s) == true;
  //@ ensures obs(nil, nil);
{
  pair<int, channel> reqch';
  while(true)
    //@ invariant obs(nil,cons(s,nil)) &*& [_]channel(s,Ms(s),(M's)(s),U);
    {
    reqch' = receive(s);
    //@ open Ms(s)(_);    
    //@ g_trandit(s);
...
void client(struct channel *s)
  //@ requires obs(nil, nil) &*& [_]channel(s,Ms(s),(M's)(s),U) &*& trandit(s) &*& Mr(s) == cons(1r,nil) &*& Ser(s) == true;
  //@ ensures obs(nil, nil);
{
  //@ close create_channel_ghost_args(1r, nil, false);
  struct channel *ch' = create_channel();
  //@ close init_channel_ghost_args<int>(Mch'(s), (M'ch')(s), U);
  //@ init_channel(ch');  
  //@ leak channel(ch',Mch'(s),(M'ch')(s),U);
  //@ g_credit(ch');
  int req = 12;
  ...
  //@ ensures obs(nil, nil);
{
  ...
  //@ close init_channel_ghost_args< pair<int, channel> >(Ms(s), (M's)(s),U);
  //@ init_channel(s);
