...
/*@
typedef lemma void invariant_preserved(struct mutex *m, predicate(fixpoint(void*, unsigned int) Wt, fixpoint(void*, unsigned int) Ot) p, list<void*> O1, list<void*> O2, list<void*> R, predicate() res)();
    requires mutex_held(m, ?f, ?Wt1, ?Ot1) &*& mutex_inv(m, p) &*& p(Wt1,Ot1) &*& obs(O1,R);
    ensures mutex_held(m, f, ?Wt2, ?Ot2) &*& mutex_inv(m, p) &*& p(Wt2,Ot2) &*& obs(O2,R) &*& res();
...
lemma void g_chrgu(condvar condvar);
    requires umutex(mutex_of(condvar), ?Wt, ?Ot) &*& obs(?O, ?R);
    ensures umutex(mutex_of(condvar), Wt, finc(Ot,condvar)) &*& obs(cons(condvar,O), R);

lemma void g_chrgu'(int n, condvar condvar);
    requires umutex(mutex_of(condvar), ?Wt, ?Ot) &*& obs(?O, ?R) &*& n >= 0;
    ensures umutex(mutex_of(condvar), Wt, finc'(n,Ot,condvar)) &*& obs(ntimes_list(nat_of_int(n),condvar,O), R);

lemma void g_chrgl(condvar condvar);
    requires mutex_held(mutex_of(condvar), ?f, ?Wt, ?Ot) &*& obs(?O, ?R);
    ensures mutex_held(mutex_of(condvar), f, Wt, finc(Ot,condvar)) &*& obs(cons(condvar,O), R);

lemma void g_dischu(condvar condvar);
    requires umutex(mutex_of(condvar), ?Wt, ?Ot) &*& obs(?O, ?R) &*& enfo(condvar,Wt,fdec(Ot,condvar))==true; //safe_obs(condvar,Wt,fdec(Ot,condvar)) == true;// &*& 0 < Ot(condvar);
    ensures umutex(mutex_of(condvar), Wt, fdec(Ot,condvar)) &*& obs(remove(condvar,O), R);

lemma void g_dischl(condvar condvar);
    requires mutex_held(mutex_of(condvar), ?f, ?Wt, ?Ot) &*& obs(?O, ?R) &*& enfo(condvar,Wt,fdec(Ot,condvar)) == true;// &*& 0 < Ot(condvar);
    ensures mutex_held(mutex_of(condvar), f, Wt, fdec(Ot,condvar)) &*& obs(remove(condvar,O), R);
...
lemma void neq_cond_mutex(struct condvar *v, void* m);
  requires [?f]cond(v,?M,?M') &*& umutex(m,?Wt,?Ot);
  ensures [f]cond(v,M,M') &*& umutex(m,Wt,Ot) &*& (void*) v != m;     
@*/

mutex create_mutex();
    //@ requires create_mutex_ghost_args(?wlevel);
    //@ ensures result != 0 &*& umutex(result, empb ,empb) &*& mutex_of(result)==result &*& level_of(result) == wlevel;
    
void mutex_acquire(mutex mutex);
    //@ requires [?f]mutex(mutex) &*& obs(?O, ?R) &*& mutex_inv(mutex, ?p) &*& object_lt_objects(mutex,O) == true;
    //@ ensures mutex_held(mutex, f, ?Wt, ?Ot) &*& p(Wt,Ot) &*& obs(cons(mutex,O), R);

void mutex_release(mutex mutex);
    //@ requires mutex_held(mutex, ?f, ?Wt, ?Ot) &*& mutex_inv(mutex,?p) &*& p(Wt, Ot) &*& obs(?O, ?R);
...
void condvar_wait(condvar condvar, mutex mutex);
    /*@ requires [?fc]cond(condvar,?M,?M') &*& mutex_held(mutex, ?f, ?Wt, ?Ot) &*& mutex_inv(mutex,?inv) &*& inv(finc(Wt,condvar), Ot) &*& obs(?O, ?R) &*&
    ...
    //@ ensures [fc]cond(condvar,M,M') &*& mutex_held(mutex, f, ?Wt', ?Ot') &*& inv(Wt', Ot') &*& obs(O, R) &*& M();

void condvar_signal(condvar condvar);
    //@ requires obs(?O, ?R) &*& [?fc]cond(condvar,?M,?M') &*& mutex_held(?mutex, ?f, ?Wt, ?Ot) &*& (Wt(condvar) <= 0 ? true : M());
    //@ ensures obs(O, R) &*& [fc]cond(condvar,M,M') &*& mutex_held(mutex, f, fdec(Wt,condvar), Ot);
