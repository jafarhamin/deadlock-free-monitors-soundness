...
Definition validk (sp: bool) (t:thrds) (h:heap) :=
  exists (T: list (cmd * context * pheap * list (olocation Z) * gheap * Z)) (invs: Z -> inv) (locs: list (location Z))
    ...
    forall id c tx p O C (ING: In (c,tx,p,O,C,id) T),
      wellformed (c,tx) /\ sat p (Some O) C (weakest_pre_ct sp (c,tx) invs) /\
...
Lemma Lookup_preserves_validity:
  forall sp h t id tx e vl
         (VALIDK: validk sp t h)
         (CMD: t id = Some (Lookup e, tx))
         (ALC: h ([[e]]) = Some vl),
    validk sp (upd Z.eq_dec t id (Some (Val (Enum vl), tx))) h.
Proof.
  intros.
  destruct VALIDK as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,(SPUR,(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
  unfold validk.
...
Lemma Mutate_preserves_validity:
  forall sp h t id tx e1 e2 v
         (VALIDK: validk sp t h)
         (CMD: t id = Some (Mutate e1 e2, tx))
         (ALC: h ([[e1]]) = Some v),
    validk sp (upd Z.eq_dec t id (Some (tt, tx))) (upd Z.eq_dec h ([[e1]]) (Some ([[e2]]))).
Proof.
  intros.
  destruct VALIDK as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,(SPUR,(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
  unfold validk.
  ...
  assumption.
  rewrite aol in *.
  apply red_Mutate with v; try assumption.
  }
  exists.
  {
  unfold spur_ok.
  ...
  apply in_map. assumption.
  apply WASW.
  destruct (location_eq_dec Z.eq_dec l v0).
  rewrite <- e in *.
...
Lemma Fork_preserves_validity:
  forall sp h t id id' c tx
         (CMD: t id = Some (Fork c, tx))
         (NIN: t id' = None)
         (VALID: validk sp t h),
    validk sp (upd Z.eq_dec (upd Z.eq_dec t id (Some (tt, tx))) id' (Some (c, done))) h.
Proof.
  intros.
  unfold validk.
...
Lemma Let_preserves_validity:
  forall sp h t id x c1 c2 tx
         (CMD: t id = Some (Let x c1 c2, tx))
         (VALID: validk sp t h),
    validk sp (upd Z.eq_dec t id (Some (c1, Let' x c2 tx))) h.
Proof.
  intros.
...
(* ==================== y6 = id ===========*)
  ...
  apply sat_weak_imp with (a:=(fun z : Z =>
            weakest_pre sp c2 (weakest_pre_tx sp tx invs)
...
Lemma Val_preserves_validity:
  forall sp h t id x e c tx
         (CMD: t id = Some (Val e, Let' x c tx))
         (VALID: validk sp t h),
    validk sp (upd Z.eq_dec t id (Some (subs c (subse x ([[e]])), tx))) h.
Proof.
  intros.
  unfold validk in VALID.
  destruct VALID as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,((SPUR1,SPUR2),(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
...
(* ==================== y6 = id ===========*)
  ...
  split. apply not_waiting_subs; assumption.
  split. apply not_waiting_subs; assumption.
  apply not_waiting_subs; assumption. assumption.
  exists.
  unfold weakest_pre_ct in *.
  assumption.
  intros.
...
Lemma Val_done_preserves_validity:
  forall sp h t id z
         ...
         (VALID: validk sp t h),
    validk sp (upd Z.eq_dec t id None) h.
Proof.
  intros.
  unfold validk in VALID.
  destruct VALID as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,((SPUR1,SPUR2),(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
...
Lemma If1_preserves_validity:
  forall sp h t id tx e c1 c2
         (VALIDK: validk sp t h)
         (CMD: t id = Some (Val e, If' c1 c2 tx))
         (TRUE: (0 < ([[e]]))%Z),
    validk sp (upd Z.eq_dec t id (Some (c1, tx))) h.
Proof.
  intros.
  destruct VALIDK as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,(SPUR,(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
  unfold validk.
...
Lemma If2_preserves_validity:
  forall sp h t id tx e c1 c2
         ...
         (CMD: t id = Some (Val e, If' c1 c2 tx))
         (False: (([[e]]) <= 0)%Z),
    validk sp (upd Z.eq_dec t id (Some (c2, tx))) h.
Proof.
  intros.
  destruct VALIDK as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,(SPUR,(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
  unfold validk.
...
Lemma If_preserves_validity:
  forall sp h t id tx c c1 c2
         (VALIDK: validk sp t h)
         (CMD: t id = Some (If c c1 c2, tx)),
    validk sp (upd Z.eq_dec t id (Some (c, If' c1 c2 tx))) h.
Proof.
  intros.
  destruct VALIDK as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,(SPUR,(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
  unfold validk.
...
Lemma Newlock_preserves_validity:
  forall sp h t id tx l
         (VALIDK: validk sp t h)
         (CMD : t id = Some (Newlock, tx))
         (FRE: h l = None),
    validk sp (upd Z.eq_dec t id (Some (Val (Enum l),tx))) (upd Z.eq_dec h l (Some 1%Z)).
Proof.
  intros.
  unfold validk in VALIDK.
...
Lemma g_initl_preserves_validity:
  forall sp h t id l tx
         (VALIDK: validk sp t h)
         (CMD : t id = Some (g_initl l, tx)),
    validk sp (upd Z.eq_dec t id (Some (tt, tx))) h.
Proof.
  intros.
  unfold validk in VALIDK.
  ...
  exists (updl T id (tt, tx, upd (location_eq_dec Z.eq_dec) (upd (location_eq_dec Z.eq_dec) p1 ll None) (Oof ll, i, Mof ll, M'of ll) (Some Lock), O, C1, id)).
  exists invs, ((Oof ll, i, Mof ll, M'of ll)::(remove (location_eq_dec Z.eq_dec) ll locs)).
  ...
  trivial.
  assumption.
  destruct ((location_eq_dec Z.eq_dec) (Oof ll, i, Mof ll, M'of ll) l0).
  rewrite <- e.
  rewrite <- EQh.
  rewrite LOCK.
  assumption.
...
Lemma Acquire0_preserves_validity:
  forall sp h t id l tx
         ...
         (NON: h ([[l]]) <> Some 1%Z),
    validk sp (upd Z.eq_dec t id (Some (Waiting4lock l, tx))) h.
Proof.
  intros.
  unfold validk in VALIDK.
  destruct VALIDK as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,((SPUR1,SPUR2),(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
...
Lemma Acquire_preserves_validity:
  forall sp h t id l tx
         ...
         (CMD : t id = Some (Acquire l, tx))
         (NON: h ([[l]]) = Some 1%Z),
    validk sp (upd Z.eq_dec t id (Some (tt, tx))) (upd Z.eq_dec h ([[l]]) (Some 0%Z)).
Proof.
  intros.
  unfold validk in VALIDK.
  destruct VALIDK as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,((SPUR1,SPUR2),(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
...
Lemma Release_preserves_validity:
  forall sp h t id l tx
         ...
         (ALC: h ([[l]]) <> None)
         (HELD: h ([[l]]) <> Some 1%Z),
    validk sp (upd Z.eq_dec t id (Some (tt, tx))) (upd Z.eq_dec h ([[l]]) (Some 1%Z)).
Proof.
  intros.
  unfold validk in VALIDK.
  destruct VALIDK as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,((SPUR1,SPUR2),(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
...
Lemma Newcond_preserves_validity:
  forall sp h t id tx v
         (VALIDK: validk sp t h)
         (CMD : t id = Some (Newcond, tx))
         (FRE: h v = None),
    validk sp (upd Z.eq_dec t id (Some (Val (Enum v),tx))) (upd Z.eq_dec h v (Some 0%Z)).
Proof.
  intros.
  unfold validk in VALIDK.
  destruct VALIDK as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,(SPUR,(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
...
Lemma g_initc_preserves_validity:
  forall sp h t id l tx
         (VALIDK: validk sp t h)
         (CMD : t id = Some (g_initc l, tx)),
    validk sp (upd Z.eq_dec t id (Some (tt, tx))) h.
Proof.
  intros.
  unfold validk in VALIDK.
  ...
  assert (tmp: exists p O C, In (g_initc l, tx, p, O, C, id) T).
  apply TOK.
  assumption.
  destruct tmp as (p,(O,(C,INT))).
  assert (tmp:=INT).
  ...
  apply sat_initc in WP; try assumption.
  destruct WP as (ll,(ml,(m'l,(lk,(wt,(ot,(eql,(pl,(plk,SAT))))))))).
  ...
  destruct (Z.eq_dec x6 id).
  rewrite e in *.
  assert (EQX: (x1, x2, x3, x4, x5) = (g_initc l, tx, p, O, C)).
  {
  eapply unique_key_eq.
  apply H.
  assumption.
  ...
  exists invs, (((Aof ll, Rof ll, Aof lk, Xof ll, Pof ll), Iof ll, ml, m'l)::(remove (location_eq_dec Z.eq_dec) ll locs)).
  exists Pinv, Pleak, Ainv, Cinv, Cleak.
  ...
  intros.
  simpl.
  assert (EQ: x' = (g_initc l, tx, p, O, C,id)).
  {
  apply in_elem with T; assumption.
  }
  rewrite EQ.
  ...
  simpl.
  destruct (Z.eq_dec id' id).
  assert (EQA: a = (g_initc l, tx, p, O, C)).
  {
  apply unique_key_eq with T id; try tauto.
  rewrite <- e.
  assumption.
  ...
  apply steps_preserve_inf_capacity with sp t h.
  assumption.
  apply red_g_initc with l; try assumption.
  }
  exists.
  {
  destruct SPUR as (SPUR0,SPUR).
...
Lemma g_finlc_preserves_validity:
  forall sp h t id v tx
         (CMD: t id = Some (g_finlc v, tx))
         (VALID: validk sp t h),
    validk sp (upd Z.eq_dec t id (Some (tt, tx))) h.
Proof.
  intros.
  ...
  assert (tmp: exists p O C, In (g_finlc v, tx, p, O, C, id) T).
  {
  apply TOK.
  assumption.
  }
  ...
  unfold wellformed in WF.
  simpl in WF.
  apply sat_g_finlc in WP.
  destruct WP as (lv,(lk,(aolv,(loflv,(plv,(plk,(spurlv,sat1))))))).
  ...
  simpl.
  destruct (Z.eq_dec a6 id).
  assert (EQA: (a1,a2,a3,a4,a5) = (g_finlc v, tx, p, O, C)).
  eapply unique_key_eq.
  apply H.
  rewrite e.
  assumption.
  ...
  rewrite EQ in *.  
  assert (EQa: (x1',x2',x3',x4',x5') = (g_finlc v, tx, p, O, C)).
  eapply unique_key_eq.
  apply IN.
  assumption.
  assumption.
  ...
  apply steps_preserve_inf_capacity with sp t h.
  assumption.
  apply red_g_finlc with v; try assumption.
  }
...
Lemma Wait_preserves_validity:
  forall sp h t id v l tx
         (CMD: t id = Some (Wait v l, tx))
         (ALCl : h ([[l]]) <> None)
         (ALCv : h ([[v]]) <> None)
         (HELD : h ([[l]]) <> Some 1%Z)
         (VALID: validk sp t h),
    validk sp (upd Z.eq_dec t id (Some (Waiting4cond v l, tx)))(upd Z.eq_dec h ([[l]]) (Some 1%Z)).
Proof.
  intros.
  unfold validk in VALID.
  destruct VALID as (T,(invs,(locs,(Pinv,(Pleak,(Ainv,(Cinv,(Cleak,(INF,((SPUR1,SPUR2),(PHsOK,(GHsOK,(TOK,(NDPT,(INVsOK,(LOCsOK,(LOCKsOK,(WTsOTsOK,SOBS)))))))))))))))))).
...
Lemma Notify_preserves_validity:
  forall sp h t id id' v v' l tx tx'
         (HV: h ([[v]]) <> None)
         (EQvv': ([[v]]) = ([[v']]))
         (CMD: t id = Some (Notify v, tx))
         (CMD': t id' = Some (Waiting4cond v' l, tx'))
         (VALID: validk sp t h),
    validk sp (upd Z.eq_dec (upd Z.eq_dec t id (Some (tt, tx))) id' (Some (Waiting4lock l, tx'))) h.
Proof.
  intros.
  unfold validk in VALID.
...
Lemma NotifyAll_preserves_validity:
  forall sp h t id v tx
         ...
         (hv: h ([[v]]) <> None),
    validk sp (upd Z.eq_dec (wakeupthrds ([[v]]) t) id (Some (tt,tx))) h.
Proof.
  intros.
  unfold validk in VALID.
...
Lemma SpuriousWakeup_preserves_validity:
  forall h t id v l tx
         (VALIDK: validk true t h)
         (CMD: t id = Some (Waiting4cond v l,tx)),
    validk true (upd Z.eq_dec t id (Some (WasWaiting4cond v l,tx))) h.
Proof.
  intros.
  unfold validk in VALIDK.
