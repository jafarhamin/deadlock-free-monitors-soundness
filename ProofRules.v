...
Definition proof_rule a c a' R I L M P X :=
 a |= weakest_pre c a' id R I L M P X.

(*
Theorem rule_Let a a'' c1 c2 x I L M P X:
  forall a'
         (RULE12: exists R', proof_rule a c1 a' R' I L M P X /\
          forall z, exists R'' (REF: refines R' R''), proof_rule (a' z) (subs c2 (subse x z)) a'' R'' I L M P X),
    exists R, proof_rule a (Let x c1 c2) a'' R I L M P X.
Proof.
  unfold proof_rule.
  simpl.
  intros.
  destruct RULE12 as (R', (RULE1,RULE2)).
  exists R'.
  intros.
  eapply sat_weak_imp.
  assumption.
  ...
  unfold id.
  simpl.
  specialize RULE2 with z.
  destruct RULE2 as (R'', (EQ, RULE2)).
  apply RULE2 in SAT0.
  apply sat_pre_subs3 in SAT0.
  apply sat_ref with R'';
  try tauto.
  assumption.
  assumption.
Qed.
...
Theorem rule_frame a a' c R I L M P X:
  forall f (RULE: proof_rule a c (fun _ => a') R I L M P X),
    proof_rule (a ** f) c (fun _ => a' ** f) R I L M P X.
Proof.
  unfold proof_rule.
  simpl.
  intros.
  destruct SAT as (p1,(p2,(phpdefp1p2,(bp1,(bp2,(o1,(o2,(C1,(C2,(opO1O2,(sat1,(sat2,(p1p2,C1C2))))))))))))). 
  assert (sata:=sat1).
  apply RULE in sata.
  rewrite <- p1p2.
  rewrite <- C1C2.
...
Theorem rule_conseq a1 a2 c R I L M P X:
  forall a1' a2' (RULE: proof_rule a1 c (fun _ => a2) R I L M P X)
         (IMP1: a1' |= a1)
         (IMP2: a2 |= a2'),
    proof_rule a1' c (fun _ => a2') R I L M P X.
Proof.
  unfold proof_rule.
  intros.
  apply sat_weak_imp with (fun _ : Z => a2).
  assumption.
  apply RULE.
  assumption.
  apply IMP1.
...
Theorem rule_Cons e R I L M P X:
  proof_rule (Abool true) (Cons e) (fun r => Apointsto 1 r ([[e]])) R I L M P X.
Proof.
  unfold proof_rule.
  simpl.
  intros.
...
Theorem rule_fork c o o' a R I L M P X:
  proof_rule (Aobs o' ** a) c (fun _ => Aobs nil) R I L M P X ->
    proof_rule (Aobs (o ++ o') ** a ) (Fork c) (fun _ => Aobs o) R I L M P X.
Proof.
  unfold proof_rule.
  simpl.
  intros.
  destruct SAT as (p1,(p2,(phpdefp1p2,(bp1,(bp2,(o1,(o2,(c1,(c2,(opo1o2,rest)))))))))).
...
Proof.
  ...
  exists r,x.
  intros.
  destruct SAT0 as (p1,(p2,(phpdefp1p2,(bp1,(bp2,(o1,(o2,(C1,(C2,(opo1o2,tmp)))))))))).
  destruct tmp as (bl,(p1v,(phpvp,bpoc))).
  split.
  assumption.
  rewrite <- phpvp in PHPDEF.
  assert (tmp: phplusdef p p1 /\ phplusdef p p2).
  apply phpdef_dist'.
  assumption.
  assumption.
  destruct tmp as (phpdefpp1,phpdefpp2).
  rewrite <- phpvp.
...
Theorem rule_g_initl R I L M P X l wt ot ct O:
  proof_rule (Abool (andb (negb (P ([[l]])))(andb (Z.eqb (L ([[l]])) ([[l]]))
...
Theorem rule_Acquire R I L M P X l O:
  proof_rule (Abool (prc ([[l]]) O R L P X) &* Alock ([[l]]) ** Aobs O) (Acquire l) 
...
Theorem rule_Release R I L M P X l O wt ot ct:
  proof_rule ((Alocked ([[l]]) wt ot ct) ** (Alasn (I ([[l]]) wt ot ct)) ** Aobs (([[l]])::O)) (Release l) 
...
Theorem rule_Newcond R I L M P X r l m b:
  proof_rule (Abool true) Newcond (fun v => Abool (andb (Z.eqb (R l) r)(andb (Z.eqb (L (v)) l)
...
Theorem rule_Wait R I L M P X l v O wt ot ct:
  proof_rule (Abool (andb (Z.eqb (L ([[v]])) ([[l]]))
...
Theorem rule_Notify R I L M P X v wt ot ct:
  proof_rule (Acond ([[v]]) ** Alocked (L ([[v]])) wt ot ct ** (Alasn (M_to_crd (M ([[v]])) ([[v]])))) (Notify v)
...
Theorem rule_NotifyAll R I L M P X v wt ot ct:
  proof_rule (Abool (negb (M ([[v]]))) &* Acond ([[v]]) ** Alocked (L ([[v]])) wt ot ct) (NotifyAll v)
...
Theorem rule_g_dupl R I L M P X l:
  proof_rule (Alock ([[l]])) (g_dupl l) (fun _ => Alock ([[l]]) ** Alock ([[l]])) R I L M P X.
Proof.
  unfold proof_rule.
  intros.
  simpl in SAT.
...
Theorem rule_g_chrg R I L M P X v wt ot ct O:
  proof_rule (Acond ([[v]]) ** Aobs O ** Alocked (L ([[v]])) wt ot ct) (g_chrg v) (fun _ => Aobs (([[v]])::O) ** Alocked (L ([[v]])) wt (incb ot ([[v]])) ct) R I L M P X.
Proof.
  unfold proof_rule.
  intros.
  simpl in SAT.
...
Theorem rule_g_chrgu R I L M P X v wt ot ct O:
  proof_rule (Acond ([[v]]) ** Aobs O ** Aulock (L ([[v]])) wt ot ct) (g_chrgu v) (fun _ => Acond ([[v]]) ** Aobs (([[v]])::O) ** Aulock (L ([[v]])) wt (incb ot ([[v]])) ct) R I L M P X.
Proof.
Admitted.

Theorem rule_g_disch R I L M P X v wt ot ct O:
  proof_rule (Abool (andb (safe_obs ([[v]]) (wt ([[v]])) ((ot ([[v]]) - 1)) R L P X) (Nat.ltb 0 (ot ([[v]]))))
...
Theorem rule_g_gain R I L M P X v wt ot ct:
  proof_rule (Acond ([[v]]) ** Alocked (L ([[v]])) wt ot ct) (g_gain v) (fun _ => Alocked (L ([[v]])) wt ot (incb ct ([[v]])) ** Acredit ([[v]])) R I L M P X.
Proof.
Admitted.

Theorem rule_g_gainu R I L M P X v wt ot ct:
  proof_rule (Acond ([[v]]) ** Aulock (L ([[v]])) wt ot ct) (g_gain v) (fun _ => Aulock (L ([[v]])) wt ot (incb ct ([[v]])) ** Acredit ([[v]])) R I L M P X.
Proof.
Admitted.

Theorem rule_g_lose R I L M P X v wt ot ct:
  proof_rule (Abool (Nat.ltb 0 (ct ([[v]]))) &* Acond ([[v]]) ** Alocked (L ([[v]])) wt ot ct ** Acredit ([[v]])) (g_lose v) 
  (fun _ => Alocked (L ([[v]])) wt ot (decb ct ([[v]]))) R I L M P X.
Proof.
Admitted.

Theorem rule_g_loseu R I L M P X v wt ot ct:
  proof_rule (Abool (Nat.ltb 0 (ct ([[v]]))) &* Acond ([[v]]) ** Aulock (L ([[v]])) wt ot ct ** Acredit ([[v]])) (g_loseu v) 
  (fun _ => Aulock (L ([[v]])) wt ot (decb ct ([[v]]))) R I L M P X.
Proof.
Admitted.

Theorem rule_g_info R I L M P X v wt ot ct:
  proof_rule (Acond ([[v]]) ** Alocked (L ([[v]])) wt ot ct ** Acredit ([[v]])) (g_info v) 
  (fun _ => Abool (Nat.ltb O (ct ([[v]]))) &* Alocked (L ([[v]])) wt ot ct) R I L M P X.
Proof.
Admitted.

Theorem rule_g_infou R I L M P X v wt ot ct:
  proof_rule (Acond ([[v]]) ** Aulock (L ([[v]])) wt ot ct ** Acredit ([[v]])) (g_info v) 
  (fun _ => Abool (Nat.ltb O (ct ([[v]]))) &* Aulock (L ([[v]])) wt ot ct) R I L M P X.
Proof.
Admitted.

Theorem rule_Lookup R I L M P X f e z:
  proof_rule (Apointsto f ([[e]]) z) (Lookup e) (fun r => Apointsto f ([[e]]) z &* Abool (Z.eqb r z)) R I L M P X.
Proof.
Admitted.

Theorem rule_Mutate R I L M P X e1 e2 z:
  proof_rule (Apointsto 1 ([[e1]]) z) (Mutate e1 e2) (fun _ => Apointsto 1 ([[e1]]) ([[e2]])) R I L M P X.
Proof.
Admitted.*)
