...
void cserver(struct channel *ch)
  //@ requires obs(nil, cons(ch,nil)) &*& [_]channel(ch,Mch,M'ch) &*& credit(ch) &*& Mr(ch) == cons(1r,nil) &*& level_of(ch) == 1r &*& Ser(ch) == false;
  //@ ensures obs(nil, nil);
{
...
void main()
  ...
  //@ ensures obs(nil, nil);
{
  //@ close create_channel_ghost_args(1r, cons(1r,nil),true);
