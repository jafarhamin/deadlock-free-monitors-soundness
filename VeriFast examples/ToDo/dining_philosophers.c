...
predicate philosopher(philosopher ph, fixpoint(void*, unsigned int) Wt, fixpoint(void*, unsigned int) Ot, mutex m, philosopher previous, philosopher next) =
  ...
  [_]previous->next |-> ph &*&
  ph != next &*& ph != previous &*& next != previous &*&
  [2/3]cond(cv) &*& Trn(cv) == no_transfer &*&
  Trn_obs(cv) == cons(previous_cv,cons(next_cv,nil)) &*&
  X(cv) == none &*& P(cv) == false &*& spur_wkup(cv) == false &*&
  ...
  //@ ensures dphs(result, ?cvs) &*& map_wlevels(cvs,wlevels) == true &*& forall(cvs,X_none) == true &*& length(cvs) == size;
{
  ...
  //@ close create_condvar_ghost_args(nth(0,wlevels),false,none,false);
  ...
  //@ close create_condvar_ghost_args(nth(size-2,wlevels),false,none,false);
  struct condvar *v1 = create_condvar();

  //@ close create_condvar_ghost_args(nth(size-1,wlevels),false,none,false);
  ...
  while(i < size)
    /*@ invariant 3 <= i &*& i <= size &*&
          ...
          forall(cvs,X_none) == true &*&      
          v0 != phs_next_cv &*&
          malloc_block_philosopher(ph2) &*&
          philosophers(phs_next_next,ph2,Wt,Ot,m);
    @*/
  {
    //@ close create_condvar_ghost_args(nth(size-i,wlevels),false,none,false);
...
void pickup(dining_philosophers dph, int i)
  //@ requires obs(?O) &*& [?fd]dphs(dph,?cvs) &*& no_cycle(nth(i,cvs),O) == true &*& 0 <= i &*& i < length(cvs) &*& wait_level_below_obs(pair({(pickup)}, 0r), O) == true;
  //@ ensures obs(cons((void*)nth((i+length(cvs)-1)%length(cvs),cvs),cons((void*)nth((i+1)%length(cvs),cvs),O))) &*& [fd]dphs(dph, cvs);
{
  ...
  @*/
    ...
    //@ close condvar_trn(cv,no_transfer);  
    //@ close condvar_obs_trn(cv,cons(previous_cv,cons(next_cv,nil))); 
...
void test(philosopher ph)
  /*@ requires obs(?O) &*& ph->state |-> ?state &*& [_]ph->cv |-> ?cv &*& [?f0]cond(cv) &*&
        ...
        mutex_held(mutex_of(cv), ?f, ?Wt, ?Ot) &*& Wt(cv) <= 1 &*& X(cv) == none &*& P(cv) == false &*& 1 + st(previous_state) + st(next_state) <= Ot(cv) &*&
        (previous_state == Thinking || next_state == Thinking) &*& (state == Hungry ? 0 < Wt(cv) : Wt(cv) <= 0) &*&
        Trn(cv) == no_transfer &*& Trn_obs(cv) == cons(previous_cv,cons(next_cv,nil)) &*&
