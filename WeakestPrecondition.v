...
Lemma prc_ref:
  forall R R' o O L P X
        (REF: refines R R')
        (DOM: dom_f R (o :: O))
        (PRC: prc o O (lift_f 0 R') L P X = true),
    prc o O (lift_f 0 R) L P X = true.
Proof.
  unfold prc.
  intros.
  ...
  unfold negb in *.
  unfold ifb in *.
  destruct (in_dec Z.eq_dec (lift_f 0 R o) (X (L o))).
  destruct (in_dec Z.eq_dec (lift_f 0 R' o) (X (L o))).
  assumption.
  simpl in *.
  exfalso.
  apply n.
  apply lift_f_in with R.
  unfold dom_f in DOM.
  apply DOM.
  left.
  reflexivity.
...
Lemma prc0_ref:
  forall R R' se v L P X
        (REF: refines R R')
        (PRC: prc0 se v R' L P X),
    prc0 se v R L P X.
Proof.
  unfold prc0.
  intros.
  assert (DOM: dom_f R' (se :: v)).
  unfold dom_f in *.
  unfold refines in *.
  intros.
  apply H in H0.
  ...
  tauto.
  apply PRC in DOM.
  apply prc_ref with R';
  try tauto.
Qed.

Lemma sat_ref:
  forall c R R' p o C a se I L M P X (BP: boundph p)
         (REF: refines R R')
         (SAT: sat p o C (weakest_pre c a se R' I L M P X)),
    sat p o C (weakest_pre c a se R I L M P X).
Proof.
  induction c;
  try tauto.
  {
  ...
  apply IHc with R';
  try tauto.
  replace p2 with (phplus p2 p').
  replace C2 with (bagplus C2 C').
  apply sat2 with O';
  try tauto.
  rewrite C'N.
  apply bplus_emp.
  ...
  simpl.
  intros.
  destruct SAT as (v,(p1,(p2,(phpdefp1p2,(bp1,(bp2,(bp12,(o1,(o2,(C1,(C2,(opO1O2,((PRC,sat1),(sat2,(p1p2,C1C2))))))))))))))).
  exists v, p1, p2, phpdefp1p2, bp1, bp2, bp12, o1, o2, C1, C2, opO1O2.
  split.
  split.
  apply prc0_ref with R';
  try tauto.
  tauto.
  split.
  intros.
  apply sat2 with v0 v1 v2 O';
  try tauto.
  tauto.
  }
Admitted.
...
Proof.
  ...
  destruct tmp4 as (p5,(p6,(phpdefp5p6,(bp5,(bp6,(bp56,(O5,(O6,(C5,(C6,(opO5O6,(tmp9,(tmp7,tmp8))))))))))))).
  intros.
  apply IMP.
  assumption.
  apply tmp5 with O'.
...
forall p O C l tx R I L M P X
  ...
  /\ prc ([[l]]) O R L P X = true /\
    forall p1 C1 wt ot ct (bp: boundph p1) (p1l : p1 ([[l]]) = Some Lock \/ p1 ([[l]]) = None) (SAT1: sat p1 None C1 (Alasn (I ([[l]]) wt ot ct))),
...
Proof.
  ...
  simpl in sat4.
  destruct sat1 as (p3,(p4,(phpdefp3p4,(bp3,(bp4,(bp34,(O3,(O4,(C3,(C4,(opO3O4,tmp1))))))))))).
  destruct tmp1 as (sat1, (sat3,(p3p4,C3C4))).
  simpl in sat1.
  simpl in sat3.
  split.
  rewrite <- p1p2.
  ...
  apply phplus_lock.
  assumption.
  assert (PERMvO: Permutation v O /\ o2 = None).
  {
  destruct sat3 as (p4N,(C4N,opvo)).
  inversion opvo.
  rewrite <- H1 in opO3O4.
  inversion opO3O4.
  rewrite <- H2 in opO1O2.
  ...
  assert (G: sat (phplus p2 (phplus (upd p1 ([[l]]) (Some (Locked wt ot ct))) p0))
    ...
    (weakest_pre_tx tx R I L M P X 0)).
  {
  apply sat2 with wt ot ct (Some (([[l]]) :: O)).
admit.
admit.
admit.
admit.
...
Proof.
  ...
  destruct SAT as (v0,(p1,(p2,(phpdefp1p2,(bp1,(bp2,(bp12,(o1,(o2,(C1,(C2,
    (opo1o2,(tmp1,(tmp2,(p1p2,C1C2))))))))))))))).
  destruct tmp1 as (bl,(p3,(p4,(phpdefp3p4,(bp3,(bp4,(bp34,(o3,(o4,(C3,(C4,(opo3o4,(tmp1,(tmp3,tmp4)))))))))))))).
...
admit.
  exists bp12.
  exists (Some O), None.
  exists empb, (bagplus C2 (fun x => if Z.eq_dec x ([[v]]) then if bool_dec (M ([[v]])) true then (S (C1 x)) else (C1 x) else (C1 x))).
  exists.
admit.
  exists.
  exists p1, (emp knowledge).
...
admit.
  ...
  destruct SAT as (p7,(p8,(phpdefp7p8,(bp7,(bp8,(bp78,(o7,(o8,(C7,(C8,(opo7o8,(tmp8,(tmp9,tmp10))))))))))))).
  destruct tmp9 as (p9,(p0,(phpdefp9p0,(bp9,(bp0,(bp90,(o9,(o0,(C9,(C0,(opo9o0,(tmp12,(tmp13,tmp14))))))))))))).
  rewrite bplus_assoc.
  apply tmp2 with v1 v2 v3 O'.
  assumption.
...
admit.
  exists (Some (([[l]]) :: v0)), None.
  exists empb, (bagplus (fun x => if Z.eq_dec x ([[v]]) then if bool_dec (M ([[v]])) true then (S (C1 x)) else (C1 x) else (C1 x)) C').
  exists.
admit.
  split.
admit.
...
admit.
  exists bp0,bp90.
  exists None, None.
  exists C9, (bagplus (fun x => if Z.eq_dec x ([[v]]) then if bool_dec (M ([[v]])) true then (S (C1 x)) else (C1 x) else (C1 x)) C0).
  exists.
admit.
  split.
  assumption.
